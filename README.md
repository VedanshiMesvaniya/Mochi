...
All three are fully local, stored in SQLite, and reachable two ways:
through chat, and through the dedicated Reminders/Tasks/Timers windows on
Mochi's right-click menu. Chat covers creating them ("remind me to call
mom at 7pm", "set a timer for 10 minutes", "remember that I need to buy
milk"), checking them ("do I have any tasks?", "what reminders do I
...
asks which one you mean rather than guessing. The right-click windows are
there as a reliable fallback for anything chat's phrasing doesn't
recognize, and for things chat has no UI for (browsing the full list,
snoozing a reminder, setting/clearing a task's deadline).
...
- **Tasks** — a simple open/done checklist. A due date is optional — add
  ...
  report at 5pm", or check "Set deadline" in the Tasks window) and it
  sorts to the front of the list, soonest first, ahead of undated tasks.
  Unlike reminders, a task with a deadline doesn't get its own
  notification/scheduler — it's a checklist entry with a date attached,
  not a timed alert.
